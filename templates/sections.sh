SECTIONS=(mode mounts workspaces python gpu egress netaccess hostroute ssh bashrc mcp-servers codex-progress local-model hermes hermes-delegate autoresume agents codex-mcp continuity claude-md settings hooks git-identity apt-extra binaries restart-policy)

declare -g -A SEC_TITLE SEC_DESC SEC_VARS SEC_APPLY SEC_PROFILE SEC_DEPS SEC_DEP_TEXT SEC_DOCTOR_ROWS
...
SEC_DESC[hostroute]='Route container egress through a host-managed forward proxy so host /etc/hosts and host DNS resolution are honored.'
...
SEC_DESC[hermes]='Off by default; third console engine (NousResearch Hermes Agent) installed as a pinned venv in the image; local OpenAI-compatible endpoint by default.'
...
SEC_VARS[hostroute]='CBOX_HOST_ROUTE_MODE CBOX_HOST_ROUTE_APPLIED CBOX_HOST_PROXY_URL CBOX_HOST_PROXY_ADDR_MODE'
...
SEC_VARS[hermes-delegate]='CBOX_HERMES_DELEGATE CBOX_HERMES_DELEGATE_PROVIDER CBOX_HERMES_DELEGATE_BASE_URL CBOX_HERMES_DELEGATE_MODEL'
...
DOCTOR_EXTRA_ROWS='codex-profile context-manifest local-model managed-dirs config-pending sessions'
