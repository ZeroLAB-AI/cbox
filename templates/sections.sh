SECTIONS=(mode mounts workspaces python gpu egress netaccess hostroute ssh bashrc mcp-servers codex-progress agents codex-mcp continuity claude-md settings hooks git-identity apt-extra binaries restart-policy)
...
SEC_DESC[netaccess]='Reach other docker containers via a SOCKS proxy on the egress container; select which networks the proxy may join (whole-network access).'
...
SEC_VARS[netaccess]='CBOX_NETACCESS_MODE CBOX_NETACCESS_APPLIED CBOX_NETACCESS_NETWORKS CBOX_NETACCESS_CIDRS CBOX_NETACCESS_SOCKS_PORT'
