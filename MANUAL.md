...
- `cbox setup --from-global` (from inside a project) or `cbox setup --local [<root>] --from-global` re-derives that project's effective config from the global profile: silent, no wizard, and a hard reset of every project-level value to what the global profile says (machine-scoped keys are never part of a project config). `cbox setup --local [<root>]` without the flag runs the per-project wizard over the existing project config instead, keeping what it does not ask about. The root defaults to the current directory. This is the standard step after deploying new cbox files; the blessing now covers both `_common.sh` and `templates/generators.sh`, so a deploy of either requires the re-bless.
