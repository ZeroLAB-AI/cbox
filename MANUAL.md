...
Local-first routing (priority 0 over priority 5): with the `hermes-local` agent selected in `CBOX_AGENTS` (which requires `CBOX_HERMES_DELEGATE=on`), the claude routing policy and session core make it priority 0 for every delegable task - extraction and summarization, bounded defect hunting, single-file or single-diff review, narrow questions over given text, and in agent mode mechanical edits with an acceptance test - while the paid tiers (worker, code-reviewer, debugger, test-runner, doc-writer, and the driver's own context) are priority 5. `agent_label_guard.py` enforces the order: while `~/.claude/agents/hermes-local.md` is installed, a spawn of one of those five paid agents is refused unless its description or prompt carries `local-skip: <reason>` (closed list: `unavailable`, `verify-failed`, `edge-case-spec`, `cross-cutting`, `owner-explanation`, `security-gate`) or `local-verify:` (the paid spawn checks a local result). Escalation agents (`alien`, `codex-*`), `security-reviewer`, the relays and the built-ins are not gated. The gate has no separate switch: deselecting `hermes-local` from `CBOX_AGENTS` removes the agent file and with it the gate. A hermes-local that is installed but unreachable (connection error) is routed around with `local-skip: unavailable` - the guard cannot probe the endpoint and does not try to.
