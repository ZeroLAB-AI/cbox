...
Run `cbox setup` to choose a setup method. Three paths are available:

- **classic** - a handful of important questions (mode, mounts, workspaces), plus a big-feature checkbox for hermes, local-model, and hermes-delegate. One confirm, no section jumping. Advanced features like wireguard are only accessible via the menu mode. Running classic on an existing config does not reset any values - it shows your current configuration as the baseline and you can review and change what you want.
- **menu** - jump straight to any section with an advanced editor. Sections can be edited in any order.
- **walk** - the linear section-by-section path through all sections, with back/jump navigation. Kept for one release.

Each path is also a direct verb, so the chooser can be skipped: `cbox setup classic`, `cbox setup menu`, `cbox setup walk`, and inside a project `cbox setup --local [<root>] menu`.

The menu is a per-section, per-key editor (`lib/cbox_settings.py`), driven by numbered line input like the hub - not a cursor TUI. The index lists sections grouped and ordered by profile (ask, then auto, then skip), each row carrying the section id, title, a short rendering of its current values, its apply class, a `[machine]` tag for machine-scoped sections, `*` when a change is staged in `pending.apply`, and `^` when the project overrides a key in it; typing `/text` filters the index by id and title. Inside a section, enum keys are picked by number and everything else is typed; every write goes through `cbox config set KEY=VALUE`, so the transaction, validators, dependency gate, machine routing and stage-and-report table are the same ones the verb uses - the menu adds no second write path. `w` hands a section with host side effects over to `cbox setup update <section>`, `b` goes back. From the index, `a` applies pending changes, `c` switches to classic, `q` quits; in an isolated project `r` resets to the global profile and `g` re-derives from it. Without a TTY on both stdin and stdout the menu prints its usage and exits instead of prompting, and without `python3` the chooser points at `cbox setup update <section>` and `cbox setup walk`.

Each section configures part of the cbox environment. Re-run individual sections later with `cbox setup update <name>`.
...
`cbox ollama gpu-check` is a separate, host-only smoke test for the machine-scoped `ollama` infra service specifically (not the general `CBOX_GPU` reservation above): it runs `docker run --rm --device nvidia.com/gpu=all <CBOX_OLLAMA_IMAGE> nvidia-smi` and reports pass or fail before you turn on `CBOX_OLLAMA_GPU=cdi`. See the `ollama` section below for details.
...
`CBOX_CODEX_MODEL` (model slug, default `gpt-5.6-terra`) and `CBOX_CODEX_EFFORT` (enum `low`/`medium`/`high`/`xhigh`/`max`/`ultra`, default `xhigh`) render as `model =` and `model_reasoning_effort =` in the generated read-only codex profile (`cbox-container.config.toml`). `CBOX_CODEX_MODEL` is validated as a plain slug (letters, digits, `.`, `-`, `_`; no whitespace, no leading `-`) since it is written into the profile TOML unescaped. These are the profile defaults picked up by every codex session in the container; a one-off `codex -m <model>` still overrides per-run.
...
Claude and Codex version pins and the shared binary volumes (`CBOX_CLAUDE_TARGET`, `CBOX_CODEX_VERSION`, `CBOX_CODEX_TARGET`, `CBOX_BINS_SCOPE`, `CBOX_BINS_HEALTH_GATE`). Installs run host-side into machine-wide volumes; runtime mounts are read-only. One install serves every project and mode.

Every install outcome is appended to `~/.config/cbox/bins.history` (never rewritten): `epoch|volume|tool|want|from_version|to_version|status|note`, one line per version change (a same-version refresh writes no line). `from_version` is the cache value recorded before the new one overwrites it, so it always reflects what was actually running before this install. `cbox bins status` prints the current per-volume `want`/installed version/staleness for claude, codex, and hermes (`off` when hermes is not enabled); `cbox bins history [tool]` prints `bins.history`, optionally filtered to one tool. `cbox doctor` also reports a `binaries` row: `claude=<v> codex=<v> hermes=<v>`, a `[LOCKED <good> (bad <bad>: <reason>)]` marker inserted right after any tool currently held (see below), the UTC time of the last health probe across all three, the UTC time of the last recorded install across all three, and the UTC time of the next autoupdate check (or `autoupdate off` / `n/a (no channel targets)` when it does not apply).

Automatic install-time health check and hold: after every fresh claude/codex/hermes install, the host runs a short health probe against the freshly installed binary inside the install container (claude/hermes: `--version` matches a dotted-version shape; codex: a `--help` precondition followed by the same JSON-RPC `initialize`/`notifications/initialized`/`tools/list` handshake used by `cbox verify`'s "Codex MCP contract" check, requiring `developer-instructions` in the output - this is the only trigger, because an unknown codex subcommand's `--help` alone always exits 0). A definitive failure (exit 2) restores the one-deep on-disk backup (or the network fallback described below) and re-probes the restored version: if the restored version also fails the same probe, the host assumes the probe itself is unreliable in the install container (for example, no `~/.codex` auth) and rolls forward to keep the new install running, with status `unreliable` and no hold. If the restored version passes, the host writes `~/.config/cbox/bins.hold.<volume>` (`version=<good>`, `bad=<bad>`, `reason=<sanitized symptom, ASCII, max 160 chars>`, `since=<epoch>`) and every subsequent `cbox` start prints `cbox: <tool> rolled back to <good> and held there - <bad> failed the start check (<reason>); autoupdate paused for <tool>, release with: cbox bins unhold <tool>`. A hold suppresses autoupdate for that tool only (an exact pin in the conf still refuses a mismatched channel as before) and is never written into `cbox.conf`. An inconclusive probe (exit 3, e.g. a timeout) never triggers a rollback - it is logged as a note and the install proceeds normally. `cbox bins check [claude|codex|hermes]` runs the same probe manually against whatever is installed right now (host-only, needs a docker run); `cbox bins unhold <tool>` removes the hold file and re-runs the install/health check for that tool immediately.

Automatic start-time gate (`CBOX_BINS_HEALTH_GATE`, default `off`): when on, every `cbox` start runs `_bins_health_gate` after `_ensure_bins` and before the background autoupdate check. For each enabled tool (claude, codex, and hermes when `CBOX_HERMES=on`) whose currently-installed version has no cached verdict yet for the exact probe body in use, it runs one `docker run --rm` with the tool's bins volume mounted `:ro` and `--entrypoint sh`, reusing the same `_CBOX_HEALTH_SH`/`CBOX_PROBE_CODEX_ARGV` snippet as `cbox bins check` and the install-time gate. A verdict is cached in `~/.config/cbox/bins.health` as `volume|version|probe_sha|verdict|epoch`, where `probe_sha` is the sha256 of the exact probe body, so a changed probe (a cbox upgrade) invalidates every prior verdict; a matching (version, probe_sha) pair costs zero docker runs on every later start. Exit 0 caches `ok`. Exit 2 (a definitive failure) runs `install-bins.sh` in rollback mode for that tool under the same `bins.lock` flock `_ensure_bins` uses - the rollback's own status line records history/cache as usual, tagged with reason `health-gate` (not `manual`) so `cbox bins history` shows whether a revert was self-triggered by the start-time gate or requested by hand with `cbox bins rollback`. Before taking the flock turn, the gate re-checks the installed version against the one it saw as unhealthy; if another session already rolled the tool back in the meantime, this call skips its own rollback instead of repeating it. Exit 3 (inconclusive) prints a one-line warning and proceeds; nothing is cached, so the next start probes again. Anything else - the docker daemon unreachable, the tool's volume missing, or any exit code outside `{0,2,3}` - is treated as probe-skip: nothing is cached and a rollback is never triggered, so a laptop with docker down never gets flagged as unhealthy. `CBOX_BINS_HEALTH_GATE` ships `off`: the owner flips it to `on` with `cbox config set` only after verifying the probe live on the host against a real 0.153.4-class binary; until then, only the manual `cbox bins check` verb exercises the probe at all. Once on, the first start after each tool upgrade pays for up to three sequential foreground probes (one per enabled tool) before the container starts - expected latency, not a hang.

Manual offline rollback: before every claude/codex install, the host takes a one-deep backup of the currently installed version - claude keeps the resolved binary at `$HOME/.local/.cbox-prev`, codex keeps the whole release directory (`~/.local/bin/codex -> standalone/current/bin/codex`, `current -> releases/<v>`) at `~/.codex/packages/.cbox-prev` - plus a copy of the pre-install `.cbox-stamp` and a completion marker, built in a `.tmp` sibling and moved into place atomically so there is always exactly one previous version. `cbox bins rollback [claude|codex|hermes]` (host-only, under the same `bins.lock` as every other `bins` verb) restores that backup: it re-points the live symlink at the backed-up version, verifies the resolved path and hash match the backup, and rewrites the stamp with the *current* want string - the channel pin never changes on rollback, so there is no refuse cascade and no rebuild. When the on-disk backup is missing or does not validate, the host falls back to re-running the vendor installer pinned to the exact previous version recorded in `bins.history` (claude gets a zero-copy fast path if the vendor still has that version's `versions/<v>` directory on disk); if that also fails, the current install is left in place with a `cbox: ... rollback could not restore any previous version` notice (fail-open, consistent with the channel fail-open ruling). Every rollback outcome is appended to `bins.history` with status `rollback`. Hermes keeps its own `.prev` directory and rollback path (see the hermes section below); its backup now survives a successful install so it too has an offline rollback point. `cbox reinstall-bins --fresh` (host-side volume wipe) or an in-container `CBOX_INSTALL_FORCE=1` reinstall (`--force`) deletes the per-tool `.cbox-prev`/`.prev` backup along with the rest of the volume - a fresh volume has no rollback point until the next install completes.

### autoupdate

Engine autoupdate (`CBOX_AUTOUPDATE`, default `on`; `CBOX_AUTOUPDATE_TTL_HOURS`, default 24). When the engine target is a channel (claude stable/latest, codex latest, hermes latest) and the TTL since the last check elapsed, cbox re-runs the host-side vendor installer into the shared bins volumes in the background at session start (same operation as `cbox reinstall-bins`), refreshing the version stamp; log at `~/.config/cbox/autoupdate.log`, including the version installed before and after the refresh for every tool it touches. Pinned versions never autoupdate. Hermes only participates when `CBOX_HERMES=on`.

A hold (see the binaries section above) suppresses autoupdate for the held tool only. If a health probe detects a bad binary and holds it, autoupdate is paused for that tool until `cbox bins unhold <tool>` is run.
...
Off by default (absent from the rendered MCP server list and refused by `cbox ai`) until configured. Wires an OpenAI-compatible endpoint (ollama, llama.cpp llama-server, vllm, or compatible) as: (1) `local-qwen`, a text-only MCP delegate exposing one tool, and (2) `local-qwen`, a `cbox ai` engine that drives `codex --oss --local-provider ollama` against the same endpoint. Set `CBOX_LOCAL_MODEL=on` plus `CBOX_LOCAL_MODEL_URL` and `CBOX_LOCAL_MODEL_NAME` via this wizard section, `cbox setup update local-model`, or `--config`; `cbox doctor` reports ACTIVE/CONFIG-ONLY/OFF. The endpoint always runs outside cbox (no GPU/CDI grant). The delegate health probe is `GET /v1/models`. `CBOX_LOCAL_MODEL_TIMEOUT_SEC` (uint, default `600`) bounds each request to the endpoint - the default was raised from an earlier ungoverned 120s because a 27B-class model on a single card runs at roughly 25-30 tokens/sec, well past 120s for a longer completion; the `local-qwen` MCP delegate's own `tool_timeout_sec` in `etc/mcp/delegates.json` is kept above this value with headroom so the outer MCP timeout never fires first. See etc/docs/LOCAL_MODEL_RUNBOOK.md for the two setup paths (sibling container vs host process), the 27B-on-24GB tuning section, and open decisions left to the operator.
...
`CBOX_HERMES_EFFORT` (enum `none`/`low`/`medium`/`xhigh`, default `medium`) is applied to hermes as `agent.reasoning_effort`, for the console engine and for the hermes-local delegate alike; a caller of the delegate can still pick a level per call through the tool's `effort` argument. `none` turns thinking off entirely. The accepted set is deliberately narrower than hermes itself accepts (hermes also knows `minimal`, `high`, `max` and `ultra`): only `none` plus the levels the local qwen template documents are offered, so a level the local endpoint does not implement never reaches it. Measured on a local 27B model, the deepest level cost about three times the wall-clock of no thinking for the same task and produced a shorter final answer, so `medium` is the default and `none` is the cheap alternative.
...
Managed-keys ownership: provider, base URL (local provider only; `/v1` appended if missing), model name, and context length (local provider only; the value of `CBOX_OLLAMA_CONTEXT_LENGTH`, so hermes and the ollama server cannot drift apart) are re-applied on every `cbox run hermes` via the official `hermes config set model.provider|base_url|default|context_length` CLI - never a copy-if-absent seed, never a hand-written YAML merge for these four keys. The cbox-side provider enum is translated at that CLI boundary: `local` becomes hermes's `custom` endpoint provider and `openai` becomes `openai-api` (hermes has no bare `openai` provider; its alias table would silently route bare `openai` to openrouter). Any other key in `~/.hermes-cbox/config.yaml` that the user or hermes itself sets is left untouched between starts, with one more managed exception: `mcp_servers` (see below), which is a hand-written top-level block replace precisely because `hermes config set`'s dotted-key CLI cannot express a nested mapping (its own model names contain dots, which collide with the dotted-key syntax) and `hermes mcp add` has no non-interactive flag for env maps, timeouts, or the enabled switch. Secrets (`.env` under `HERMES_HOME`) and Nous OAuth login are manual, host-operator steps: `cbox shell` into the running container, then run the relevant `hermes` auth/config command by hand.

MCP servers for hermes: cbox's MCP registry (`etc/mcp/delegates.json`, rendered by `etc/mcp/render_mcp.py`) now has a third render target alongside `claude` and `codex`. `regen_all` writes the rendered set to `generated/hermes/mcp_servers.yaml` (mounted read-only into the container next to `managed.env`, at `/etc/cbox/hermes-managed/`) whenever `CBOX_HERMES=on`; on every `cbox run hermes`, the entrypoint replaces the `mcp_servers:` top-level block in `~/.hermes-cbox/config.yaml` with that file's contents (a block replace, not a general YAML parse - cbox owns the entire block it writes, so no other key in the file is touched). An entry only reaches hermes if its `_cbox.available_to` list in `delegates.json` names `hermes` explicitly; opted-in today are the five `codex-*` tiers, `ask-claude`, `local-qwen`, `container-exec`, and `hermes-local` - each a decision made by that entry's owner, not a blanket default. `timeout` (hermes defaults to 300s) is carried through explicitly from the entry's `tool_timeout_sec` so long-running tools do not silently hit hermes's own default. An entry gated by `enabled_when_env` that names `hermes` but whose gate is currently unset renders with hermes's native `enabled: false` rather than being omitted, so `hermes mcp list` shows it as present-but-off instead of it simply not existing; `ask-claude` carries no such gate, so it renders unconditionally (always present, never disabled:false) for the hermes target - accepted as intentional: hermes already has shell reach in this container through the codex tiers, and `ask_claude_mcp.py` enforces its own cwd/scope/git checks and refuses recursive delegation independently of any MCP-level gate, so gating it further behind an env var would add an opt-in switch without closing a real hole. Caveat that matters: each configured server becomes its own `mcp-<server>` toolset, auto-generated by hermes - this is a *different* mechanism from `agent.disabled_toolsets` (the knob the hermes-delegate section's qa mode uses to strip its own default toolset list). A tool given to hermes through `mcp_servers` is not covered by that strip list at all, so opting an entry in here can silently bypass a toolset restriction set up elsewhere for the same hermes instance.
...
Refresh safety: when `cbox reinstall-bins` or autoupdate refreshes the hermes venv, a failed refresh restores the previous venv from an in-volume backup (rollback via a `.prev` directory). A successful install now keeps that backup instead of deleting it, so the previous venv stays available for a manual `cbox bins rollback hermes` (offline, no network); the next install's own backup-take rotates it out (never more than one previous version on disk).
...
Machine-scoped (`SEC_SCOPE[hermes-delegate]=machine`, like `local-model` and `ollama`): decided once per host, written to the global config by `cbox config set` from any scope, stripped from project configs on their next re-bless, and read by every project at run time. It needs the `hermes` console engine, which stays project-scoped (`CBOX_HERMES=on`): instead of a dependency that forces the machine value off, the `hermes-local` entry is gated on both `CBOX_HERMES_DELEGATE` and `CBOX_HERMES` (`enabled_when_env` in `delegates.json` and `capabilities.json`), so a project without hermes simply does not render the tool while every project with hermes gets it. Set `CBOX_HERMES_DELEGATE=on` plus optional `CBOX_HERMES_DELEGATE_PROVIDER`, `CBOX_HERMES_DELEGATE_BASE_URL`, and `CBOX_HERMES_DELEGATE_MODEL` (default-inherited from the console engine's own `CBOX_HERMES_PROVIDER`/`CBOX_HERMES_MODEL_URL`/`CBOX_HERMES_MODEL_NAME` at ask-time, but stored and applied independently) via this wizard section, `cbox setup update hermes-delegate`, or `--config`. This is a restart-class change (`SEC_APPLY[hermes-delegate]=restart`), same as the other MCP delegates.
...
Two modes (`CBOX_HERMES_DELEGATE_MODE`, empty means `qa`). qa pins the hermes child's terminal, file, web, code_execution, delegation, browser and computer_use toolsets off per call before the prompt runs, so the model answers from the prompt alone. agent turns the child into an autonomous agent inside the workspace: the subprocess starts in the project root (`CBOX_SCOPE_ROOT`, falling back to the server's own cwd), terminal and file tools stay on, while code_execution (arbitrary in-process python that never passes the terminal guard), web (content-driven fetches, an exfiltration and SSRF channel), delegation, browser, computer_use and cronjob are pinned off, and the call refuses to run unless the hermes guard hooks block rendered by `CBOX_HERMES_HOOKS=on` exists read-only at `/etc/cbox/hermes-managed/hooks.yaml`, carries at least one `pre_tool_call` hook, and every absolute python script its commands name exists as a read-only regular file (the same preflight the console entrypoint runs). The block is copied into the ephemeral home's `config.yaml` as the `hooks` key together with `hooks_auto_accept: true`, and the child gets `HERMES_ACCEPT_HOOKS=1` - hermes registers shell hooks before the one-shot runner and, on a non-interactive stdin, silently skips them unless acceptance is explicit, the same reason the console entrypoint exports that variable. Those cbox guards are the only gate: hermes' own dangerous-command approval is off in one-shot mode (`-z` runs yolo), and the bridge matches both the `terminal` tool and the `process` tool's `write`/`submit` actions, since stdin sent to a background shell would otherwise bypass the rm guard. Without the block or its scripts agent mode fails closed and names the setting. The child's working directory must be inside a git work tree (and never `/` or the home directory), otherwise the call is refused rather than silently running in the server's own directory. The delegation-depth marker in the child's environment is advisory: the delegation toolset is off, but a terminal can still start another engine, so the tool description says so instead of promising the child cannot fan out. The hooks file and guard scripts count as writable when world-writable, or when the delegate runs unprivileged and `access(W_OK)` says so; a root delegate (the rootless container maps the host user to root) trusts the read-only bind mount instead of the mode bits. The audit line records the mode. Either floor can only be extended through `CBOX_HERMES_DELEGATE_DISABLED_TOOLSETS`, never shrunk. The pin is written into the ephemeral home's `config.yaml` as a genuine YAML list under `agent.disabled_toolsets` (through the hermes venv's own python and PyYAML, which hermes itself uses to read the file), never via `hermes config set`: that CLI stores a comma-separated value verbatim as a string, hermes then iterates the string character by character and disables nothing, and a plain `hermes config get` echoes the same string back so a text comparison cannot notice. The delegate therefore verifies the type, not the text: it reads the key back with `hermes config get agent.disabled_toolsets --json` and refuses the call unless the result is a JSON list carrying every pinned name. The default disabled set is `terminal,file,web,code_execution,delegation,browser,computer_use` - every toolset that could touch the machine, escape the delegation-depth guard, or drive a browser/desktop, since the whole point of qa mode is that the delegate cannot touch the machine. `CBOX_HERMES_DELEGATE_DISABLED_TOOLSETS` can only add to that list, never shrink it - the seven names above are a mandatory floor unioned in regardless of what the var is set to; this is a config-level restriction enforced by hermes itself, not a sandbox around the process - the hermes child still runs with the container's own filesystem and network reach.
...
Local-first routing (priority 0 over priority 5): with the `hermes-local` delegate available (the `hermes-local` MCP tool, which requires `CBOX_HERMES_DELEGATE=on` and `CBOX_HERMES=on`), the claude routing policy and session core make it priority 0 for every delegable task - extraction and summarization, bounded defect hunting, single-file or single-diff review, narrow questions over given text, and in agent mode mechanical edits with an acceptance test - while the paid tiers (worker, code-reviewer, debugger, test-runner, doc-writer, and the driver's own context) are priority 5. `agent_label_guard.py` enforces the order: while the local tier is available - `CBOX_HERMES_DELEGATE=on` OR `~/.claude/agents/hermes-local.md` is installed - a spawn of one of those five paid agents is refused unless its description or prompt carries `local-skip: <reason>` (closed list: `unavailable`, `verify-failed`, `edge-case-spec`, `cross-cutting`, `owner-explanation`, `security-gate`) or `local-verify:` (the paid spawn checks a local result). Escalation agents (`alien`, `codex-*`), `security-reviewer`, the relays and the built-ins are not gated. The local tier being available is the gate, not the selected agent: selecting `hermes-local` in `CBOX_AGENTS` is the recommended way to also get the agent (so a spawn can name it directly), but the gate does not depend on the agent file - it is keyed to the delegate being enabled. To switch the gate off, turn the delegate off (`CBOX_HERMES_DELEGATE=off`). A local tier that is enabled but unreachable (connection error) is routed around with `local-skip: unavailable` - the guard cannot probe the endpoint and does not try to.
...
Because the section is machine-scoped: the isolated per-project wizard (`run_local_wizard_subset`) never calls `step_ollama`, `cbox setup --local <root>` never asks about it and never writes `CBOX_OLLAMA_*` into a project's effective `cbox.conf` (the isolated derivation path strips those keys after `conf_save`), and `cbox config set` writes `CBOX_OLLAMA_*` to the machine-level `cbox.conf` no matter which scope you run it from, because the service is shared by every project (a single call mixing machine-scoped and project-scoped keys is refused, since the two land in different files). The isolated runtime path (`cbox run`/`cbox shell` in an isolated project) re-reads `CBOX_OLLAMA_*` from the machine-level `cbox.conf` after sourcing the per-project file, so every project observes the same live value rather than a stale per-project copy.

Vars: `CBOX_OLLAMA_MODE` (`off|on`, default `off`), `CBOX_OLLAMA_IMAGE` (pinned image reference, default `ollama/ollama:0.33.3` - the floor matters: Qwen3.8-class models need ollama 0.32.12 or newer, so anything below that cannot even pull the model), `CBOX_OLLAMA_GPU` (`off|cdi`, default `off` - a separate reservation from `CBOX_GPU`, targeting only the ollama service), `CBOX_OLLAMA_STORE` (`dedicated|shared`, default `dedicated` - a cbox-owned named volume/directory; `shared` mounts only the `models/` subdirectory of a host ollama directory read-write and refuses to start while a host ollama daemon is detected), `CBOX_OLLAMA_STORE_PATH` (host path, shared mode only, default empty), `CBOX_OLLAMA_PORT` (default `11434`; informational only - it is never published on the host, it is only the port the shared-store host-daemon probe checks), `CBOX_OLLAMA_NUM_PARALLEL` (default `1`), `CBOX_OLLAMA_CONTEXT_LENGTH` (uint with a 2048 floor, default `65536`, rendered as `OLLAMA_CONTEXT_LENGTH` and mirrored into hermes as `model.context_length` through `generated/hermes/managed.env` and the hermes-local delegate, so hermes budgets against the real server window instead of a guessed one - hermes documents a 64000-token minimum for agent use with tools and only warns below it, while ollama silently truncates any prompt longer than this value, so keep it at 65536 or above whenever hermes drives the model), `CBOX_OLLAMA_FLASH_ATTENTION` (`off|on`, default `on`, rendered as `OLLAMA_FLASH_ATTENTION=1` when on and `=0` when off - never left unset, because ollama reads an unset var as auto and auto resolves to on for the qwen3/qwen35/gemma3-class engines, so an omitted line would make `off` a dead value; `q8_0`/`q4_0` KV cache quantization only takes effect while flash attention is on, with `off` the cache silently falls back to f16), `CBOX_OLLAMA_KV_CACHE_TYPE` (`f16|q8_0|q4_0`, default `q8_0`, rendered as `OLLAMA_KV_CACHE_TYPE`), `CBOX_OLLAMA_KEEP_ALIVE` (an ollama keep-alive in any form ollama's own parser accepts - a duration string such as `30m` or `1h30m`, plain seconds such as `3600`, `0`, or any negative value such as `-1` for forever - default `30m`, rendered as `OLLAMA_KEEP_ALIVE`). The last four exist to make a 27B-class model at Q4 fit and run well on a single 24 GB card - see the local-model runbook's "running a 27B-class model on a single 24 GB card" section for tuning guidance.

`cbox ollama gpu-check` is a host-only preflight verb, wired the same way as the other `cbox ollama` verbs (same dispatcher, same machine lock taken in shared mode, `_cbox_ollama_guard` exempt so it can run before `CBOX_OLLAMA_MODE` is even turned on): it runs `docker run --rm --entrypoint nvidia-smi --device nvidia.com/gpu=all <CBOX_OLLAMA_IMAGE>` and reports PASS or FAIL (the entrypoint override is required because the ollama image's entrypoint is the ollama CLI, which rejects `nvidia-smi` as an unknown subcommand). On FAIL it names the fix candidates: on rootless docker, restart the daemon once after `/etc/cdi` is first created or after `nvidia-persistenced` (re)creates `/run/nvidia-persistenced` (rootlesskit runs the daemon with `--copy-up=/etc` and `--copy-up=/run`, so it sees a snapshot of both from its own start time and a directory created later is invisible to it - the symptoms are `unresolvable CDI devices nvidia.com/gpu=all` despite a world-readable spec file, or `failed to fulfil mount request: open /run/nvidia-persistenced/socket` while the socket exists on the host; `systemctl --user restart docker` resolves both, and later edits inside an already-existing directory need no restart); a missing on-demand device node (`failed to stat CDI host device /dev/nvidia-uvm`), which the first CUDA client creates after driver load or an eGPU replug while `nvidia-persistenced` alone does not - the ollama start paths create it themselves now (see below), so on those paths this candidate only survives on a host that ships neither `nvidia-modprobe` nor `nvidia-smi` - `gpu-check` itself runs the raw probe with no preflight on purpose, so it still reports the untouched truth about a freshly booted host; regenerate the CDI spec (`nvidia-ctk cdi generate`, needed again every time an eGPU is unplugged and replugged, since its PCI address changes); on a docker daemon older than 28.3 (CDI is on by default from 28.3), enable CDI in the rootless daemon's own config; and, only if you deliberately run ollama through the legacy nvidia runtime hook (`--runtime nvidia` / `--gpus`) instead of CDI, `no-cgroups = true` in the nvidia-container-runtime config - that setting is not part of the CDI path this verb tests. It never runs automatically - no other flow calls it, and it never changes `CBOX_OLLAMA_GPU` or any other config.
...
Owner self-heal on engine start: every engine start (`cbox run <bin>` and the `claude`/`codex`/`hermes` aliases, `cbox shell`, `cbox up`) checks the owner container right before starting its own compose project, under the same machine lock as the `ollama` verbs, and only when `CBOX_OLLAMA_MODE=on` and the owner project is rendered. A container that exists but is not running is started; if the start fails because its network no longer exists (the docker daemon was restarted, so the container still points at the old network id), the dead container is removed and the owner project is reconciled in place; if it fails on a rootless copy-up symptom (`/run/nvidia-persistenced/socket` missing or `unresolvable CDI devices`, both caused by the daemon still seeing the `/etc` and `/run` snapshot from its own start), the run asks `restart the docker daemon now? [Y/n]`, warns that this stops every container on the daemon including other cbox sessions, restarts it with `systemctl --user restart docker`, waits for it to answer, and reconciles. The check runs before the session's own container comes up on purpose - a daemon restart after it would kill the session that asked for it. It never touches a project that is off, never rendered, or whose owner container was stopped by `cbox ollama down`, and a failure never blocks the session: it prints the manual command and the session continues without the local model. Without a terminal nothing is restarted, only the command is printed. The explicit verbs `cbox ollama reconcile` and `cbox ollama up` run the same copy-up recovery when their own compose up fails on that symptom (the same explanation, the same prompt on a terminal, the same rootful hint) and then retry the compose up once after the restart instead of reporting only a bare `containers failed to start`.

The one case where a missing container is still created is the heal's own doing: before removing a dead container on the stale-network path it writes `.heal-recreate` in the owner directory, and clears it as soon as a start or reconcile succeeds. `cbox ollama up` and `cbox ollama reconcile` arm the same marker on their own stale-network path. If the follow-up start or reconcile fails, the marker survives, and the next engine start recreates the container instead of finding no container and returning silently - which would otherwise leave the owner dead for good, since every later heal would be a no-op. `cbox ollama down` and a de-rendering `cbox ollama reconcile` clear the marker too, so an operator taking the owner down is never overruled.

On-demand CDI device node: with `CBOX_OLLAMA_GPU=cdi` every path that claims the GPU (`cbox ollama up`, `cbox ollama reconcile`, the engine-start self-heal, and `cbox ollama pull` - both its temporary pull container and the restart of the serving one at the end) first checks that `/dev/nvidia-uvm` and `/dev/nvidia-uvm-tools` exist. The driver creates them lazily for the first CUDA client, so after a boot or an eGPU replug they are absent while the CDI spec still lists them, and the container start fails with `failed to stat CDI host device "/dev/nvidia-uvm"` until something touches CUDA on the host. cbox creates them itself with `nvidia-modprobe -u -c 0` (setuid, no root needed), falling back to `nvidia-smi -L`, and says so. The preflight never fails a start on its own - a spec that does not list those nodes is legitimate, and refusing there would block a start that would otherwise work - so if the start still fails on `CDI host device`, the creation is attempted once more and the start retried exactly once; when neither tool can create the node, the failure names `cbox ollama gpu-check` instead of leaving the raw daemon error.

Networking: one `internal:true` docker network per scope (`cbox-ollama-u<uid>-global`, `cbox-ollama-u<uid>-p<projecthash>`), never one shared network across scopes - `internal:true` blocks external routing but not member-to-member traffic, so a single shared network would let unrelated cbox containers reach each other's ollama. Each network joins exactly one cbox container plus the one ollama container, with the ollama container aliased `ollama` on each, so the endpoint is always `http://ollama:11434` regardless of scope. Both attachments are runtime `docker network connect` calls, re-applied for every running scope container by each `cbox run`, `cbox shell`, `cbox up`, `cbox ollama up` and `cbox ollama reconcile`, so an owner recreate (the heal's stale-network path) is repaired by the next engine start without any operator step. The stale sweep (disconnecting ollama from a per-scope network whose cbox side is gone) and the gc (removing per-scope networks with no members) only ever consider networks named under `cbox-ollama-u<uid>-p` (gc also the `-global` one); the owner project's own compose `default` network carries the same `cbox.component=ollama-net`/`cbox.owner` labels and is never disconnected or removed - removing it while ollama is stopped is exactly what made the next owner start fail with `network not found` and forced a recreate that dropped every attachment. A failed connect prints the docker daemon's own reason after `failed to attach`, not a bare failure line. Neither the egress network nor the tinyproxy/dante proxy carries ollama traffic; a per-scope model network's service name goes into `NO_PROXY` always (including under egress lockdown, since a direct route genuinely exists there), while external endpoints (a host IP, a remote tunnel IP) are never added to `NO_PROXY` under egress lockdown. The `NO_PROXY` entry is the bare token `ollama`; some clients (e.g. Python's `urllib`) do a no-dot-boundary suffix match, so a hostname ending in `ollama` would also be treated as proxy-exempt - under lockdown that only means the direct attempt fails (no route exists to it), so this is not currently an escalation, but the exemption must never be widened to a wildcard/pattern.
...
Because the section is machine-scoped, the same rules apply as for `ollama`: the isolated per-project wizard never calls `step_wireguard`, `cbox setup --local <root>` never writes `CBOX_WG_*` into a project's effective `cbox.conf`, and `cbox config set` writes `CBOX_WG_*` to the machine-level `cbox.conf` from any scope, the same way as `CBOX_OLLAMA_*`.
...
After the initial wizard run, re-run specific sections or the full setup with the following commands:

- `cbox setup update <section>` re-runs one section and regenerates related outputs.
- `cbox setup update` (no section) re-renders all artifacts and re-blesses the templates (CBOX_TPL_SHA) without changing configuration.
- `cbox setup --from-global` (from inside a project) or `cbox setup --local [<root>] --from-global` re-derives that project's effective config from the global profile: silent, no wizard. The project's config is layered - `<effdir>/cbox.base` is a whitelist snapshot of the global profile's project-scoped keys as of the last derive, and `<effdir>/cbox.override` holds only the keys this project explicitly changed. A derive keeps every override, except a key whose global value changed since the last derive: that one override is dropped (logged to `<effdir>/override.dropped.log` and reported on stdout), and the key falls through to the new global value. The first derive (or first touch) after upgrading a pre-layered project adopts every existing difference from the global profile as an override (nothing is silently reset); a hand-edited or otherwise drifted effective config still refuses and points at manual review. `cbox setup --local [<root>] --from-global --reset` drops every project override first and then derives, so the project ends up exactly on the global profile. `cbox setup --local [<root>]` without `--from-global` runs the per-project wizard over the existing project config instead, turning whatever it changes into overrides and leaving `cbox.base` untouched. The root defaults to the current directory. Machine-scoped keys are never part of a project config or a project override. This is the standard step after deploying new cbox files; the blessing now covers both `_common.sh` and `templates/generators.sh`, so a deploy of either requires the re-bless.
...
Transaction (isolated mode): an exclusive `flock` on `<effdir>/.regen.lock` is held for the whole operation. `cbox.conf`, the `generated/` directory, and `cbox.override` are backed up first; the new config is written atomically (temp file + `mv`), re-sourced, and the same regeneration path the engine-start flow uses runs against it, and every staged key becomes (or updates) an entry in `cbox.override` - even a value equal to the current global value, which holds the key as an override rather than dropping it. On any failure all three backups are restored verbatim and the failure is reported - `cbox.conf`, `generated/`, and `cbox.override` are left byte-identical to their pre-set state, and no manifest or `pending.apply` is touched. On success the backups are dropped and both manifest subsystems are stamped in order: the config manifest (`_cbox_manifest_write`) first, then the generated-artifacts manifest (`_cbox_manifest_write_generated`) - manifests are written last and prove file integrity, not that a running container has picked up the change. `pending.apply` is then written with one `section=apply-class` line per touched section, and a stage-and-report table is printed naming, per touched section, its apply class and the exact command to run: `none` takes effect on the next `cbox run`; `shell` needs `source ~/.bashrc`; `restart` needs `cbox down && cbox run <bin>`; `recreate`/`topology` need `cbox down && cbox run <bin>` (compose recreates); `rebuild` rebuilds the image automatically on the next `cbox run` (image.inputs changed). A malformed, colliding, or drifted manifest refuses the set outright and points at `setup.sh --local --from-global`; the same refusal covers a `cbox.base`/`cbox.override` pair that exists but whose sha256 no longer matches the manifest (edited outside cbox). A single-key edit never routes through the re-bless path, which re-derives the whole profile and drops only the overrides whose global value moved - it never clobbers the rest of a project's overrides.
...
`cbox config diff` (isolated mode only) lists every key currently held in `cbox.override`, one line each: `KEY=<override value>  base=<base[K]>  global-now=<the current global value>`, plus a trailing `CONFLICT` marker on any key whose `global-now` no longer matches `base[K]` - that marker names exactly the keys the next `cbox setup --local --from-global` would drop, computed from the same old-base-vs-new-base comparison the derive merge itself uses, not a second reimplementation. An override whose value happens to equal the current global value is never hidden - it renders as `KEY= global  base=...  global-now=...` instead of the literal value, because an explicit set is still an override and still holds the key against a future derive that moves the global value elsewhere. A project with no `cbox.base`/`cbox.override` yet (never adopted, or reset) reports plainly that there are no overrides. Like `config set`, `diff` refuses a `cbox.base`/`cbox.override` pair that exists but does not match the manifest (edited outside cbox) and is host-only.

`cbox config unset KEY [KEY...]` drops one or more keys back out of `cbox.override`: the value becomes `base[K]` when the key is present in `cbox.base` (the normal case - `cbox.base` is a full snapshot, so this is almost always where the value comes from), or the current global value when the registry gained the key after the project's last derive and `cbox.base` predates it. A key that is not currently an override is a clean no-op with a plain message and never touches the config-set transaction. Unsetting keys that are overrides runs through the exact same transaction `config set` uses - same flock, same backup/restore of `cbox.conf`/`generated/`/`cbox.override`, same dependency gate, same validators, same stage-and-report table - the only difference is that the touched keys are removed from `cbox.override` at the end instead of being (re)written into it. Isolated mode only, host-only, same layered-drift refusal as `diff` and `set`.

`cbox config get` never changes shape for either verb above - its `KEY=VALUE` output stays byte-identical, since scripts parse it.

## Operational commands

- `cbox ls` - list running isolated project containers: path hash, image hash, root directory.
- `cbox bins {status|history [tool]|check [tool]|rollback [tool]|unhold <tool>}` - reports on, health-checks, and manually rolls back the shared claude/codex/hermes binary volumes; takes the same `bins.lock` flock as `cbox reinstall-bins` (`rollback` and `unhold` take it exclusively, `status`/`history`/`check` take it shared). Status prints, per tool, the volume name, the configured `want` string, the installed version (`unknown` if never installed), and staleness (`fresh`/`stale`); hermes shows `off` when `CBOX_HERMES` is not `on`. History prints `~/.config/cbox/bins.history` (append-only, one line per recorded version change), optionally filtered to one tool. Check runs the install-time health probe against whatever is installed right now (all enabled tools, or just the named one), host-only since it needs a docker run. Rollback restores the one-deep on-disk backup for the named tool (or claude+codex, plus hermes if enabled, when no tool is given), falling back to a network reinstall pinned to the exact previous version from history when the backup is missing or invalid; it never moves the configured `want`/channel, so it never triggers the pin-mismatch refuse cascade. Unhold removes a tool's `bins.hold.<volume>` file and immediately re-runs its install/health check. See the "binaries" wizard section above for the file format, the health probe, and the rollback mechanics.
- `cbox config get/set/unset/diff/pending` - headless per-key settings; see "cbox config" above.
...
- `cbox ollama {status|up|down|pull <model>|reconcile|gpu-check}` - machine-scoped ollama owner project control (host-side only, applies `SEC_APPLY[ollama]=infra-reconcile`). Status shows the current state (OFF/CONFIG-ONLY/ACTIVE). Up/down start or stop the owner project. Pull stops the serving container, then downloads the model in a temporary ephemeral container on its own routable network (deliberately not internal, so the registry is reachable, and torn down right after the pull), then restarts the server. Both the owner service's healthcheck and the pull's readiness wait probe through the in-image `ollama ls` CLI, because the `ollama/ollama` runtime image ships neither `curl` nor `wget`. Reconcile creates, updates, or tears down the owner project to match the current `CBOX_OLLAMA_*` configuration and per-scope networks. Gpu-check is a host-only preflight smoke test (guard-exempt, shared-locked) that runs `nvidia-smi` in an ephemeral container against the configured `CBOX_OLLAMA_IMAGE` to verify CDI GPU passthrough before turning on `CBOX_OLLAMA_GPU=cdi`; see the prose section below for details.
- `cbox doctor` - report configuration status and active features inside the container.
- `cbox doctor` also reports a `stale-binds` row. A file bind mount is pinned to its source inode, so re-blessing a container (or any rewrite that replaces a bound file rather than editing it in place) leaves the running container reading the OLD content while the host disk already holds the new one; `/proc/self/mountinfo` marks those entries `//deleted`. The row is WARN with the count and an example mount point when any are stale, ACTIVE when none are, and HOST-CHECK outside a container. Restart and rebuild do NOT clear this state - the container has to be removed and created again. The same check runs at session start: `continuity_session_start.py` emits a `stale-binds` payload when it finds any, and stays silent otherwise.
...
`config` opens a small submenu: show all (`cbox config get --all`), set one key (prompts `KEY=VALUE`, then runs `cbox config set` and shows its stage-and-report output), pending (`cbox config pending`), and back; in an isolated project it gains two more lines, diff (`cbox config diff` - project overrides against base/global, with the `CONFLICT` marker) and unset a key (prompts `KEY`, then runs `cbox config unset` and shows its output).
...
Mode and scope resolution stay bash's job: the python hub calls a hidden `cbox __hub_context` to get one JSON line (mode, workspace root, effective directory, config path, the exact `docker compose` argv for this scope, service name, egress mode, a read-only `bins` status string) computed by the same `_cbox_effective_mode`/`_cbox_workspace_root`/`_first_run_init` bash uses for every other command - so mode logic is never duplicated or allowed to drift between the two hubs. `__hub_context` prints `{"mode": "none"}` when there is no workspace or no config yet and non-interactive init was refused; the python hub degrades to the usage text in that case, same as bash.

The `bins` field (`"bins: claude 2.1.0 codex 0.153.4 [held] hermes 0.9.1"`) renders straight from the on-disk `bins.stamp`/`bins.hold.<volume>` caches (`_bins_hub_line`/`_bins_hub_tool_field`) - `unknown` when a tool has no cache entry yet, `off` for hermes when `CBOX_HERMES` is not on, and a trailing `[held]` when that tool's volume has a hold file. Like every other `__hub_context` field, computing it never runs docker and never writes a file - it is a read-only probe, consistent with the context handshake's no-side-effects rule.
...
H1 screens: the main hub screen (header line, container/egress/engines status rows, one numbered row per enabled engine from `etc/engines/engines.json` plus shell/logs/doctor/config/down/quit) and its four action targets - engine rows and shell/logs/doctor/down all `exec` the existing bash verb (`cbox run <engine>`, `cbox shell`, `cbox logs`, `cbox doctor`, `cbox down`) as a subprocess, so the direct run path and every write-capable verb still live in bash unchanged. `config` in H1 is read-only: it parses `KEY=VALUE` lines directly out of the resolved `cbox.conf` (no sourcing, no section metadata) and prints them - no `cbox config set`, no netaccess/ollama/wg/session-broker/sessions submenus, no apply staging; in an isolated project it appends one more line pointing at `cbox config diff`/`cbox config unset KEY` (still informational text, not a submenu). Those stay bash-only for now and are reachable by leaving the hub (`q`) and running the verb directly, or by falling back to the bash hub's fuller submenu tree.
