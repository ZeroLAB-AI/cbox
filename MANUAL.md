...
`CBOX_HERMES_EFFORT` (enum `""`/`none`/`low`/`medium`/`xhigh`, default empty) is applied to hermes as `agent.reasoning_effort`, for the console engine and for the hermes-local delegate alike. Empty leaves the decision to the model's own default; `none` turns thinking off entirely. The accepted set is deliberately narrower than hermes itself accepts (hermes also knows `minimal`, `high`, `max` and `ultra`): a Qwen3.x chat template raises on anything outside `low`/`medium`/`xhigh`, and that surfaces as an HTTP 500 from the endpoint rather than as a config error. Measured on a local 27B model, the deepest level cost about three times the wall-clock of no thinking for the same task and produced a shorter final answer, so `medium` or `none` is usually the better trade.
