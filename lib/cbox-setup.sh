[ -n "${_CBOX_SETUP_LIB_LOADED:-}" ] && return 0
_CBOX_SETUP_LIB_LOADED=1

: "${INSTALL_DIR:=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
if [ ! -f "$INSTALL_DIR/_common.sh" ]; then
  echo "cbox setup: error: $INSTALL_DIR/_common.sh missing" >&2
  return 1
fi
...
CLAUDE_MD_USERPOL_MARK_START="<!-- cbox:user-policies:begin -->"
CLAUDE_MD_USERPOL_MARK_END="<!-- cbox:user-policies:end -->"
...
print_settings_help() {
  ...
  printf '\n'
  hr
  printf '%scbox setup sections reference%s\n' "$C_HEAD" "$C_RESET"
  hr
...
bashrc_block_file() {
  local out="$1" rc_src="${2:-\$HOME/.bashrc-cbox}"
  {
    printf '%s\n' "$MARK_START"
    printf '[ -f "%s" ] && . "%s"\n' "$rc_src" "$rc_src"
    printf '%s\n' "$MARK_END"
...
merge_bashrc_block() {
  local file="$1" rc_src="${2:-\$HOME/.bashrc-cbox}" block tmp starts
  block="$(mktemp)"
  bashrc_block_file "$block" "$rc_src"
  tmp="$(mktemp)"
  starts="$(grep -cF "$MARK_START" "$file")"
  if [ "$starts" -gt 1 ]; then
    note "warning: '$file' has $starts cbox marker starts - refusing to merge (would drop content between them); remove the extra marker block by hand and re-run"
    rm -f "$block" "$tmp"
    return 1
  fi
...
_install_rc_block() {
  local rc="$1" rc_cbox="$2" rc_cbox_src="$3" stage_cbox="$4" old
  local stage
  stage="$(mktemp)"
  staged_write "$rc_cbox" "$stage_cbox" 0644 || true
  if [ -f "$rc" ]; then
    cp "$rc" "$stage"
  else
    : > "$stage"
  fi
  if ! merge_bashrc_block "$stage" "$rc_cbox_src"; then
    rm -f "$stage"
    return 1
  fi
  old="$(bashrc_old_lines "$stage")"
  if [ -n "$old" ] && [ "$SEC_AUTO" != 1 ]; then
    note "old cbox shell block detected outside the marker block in $rc:"
    printf '%s\n' "$old" | sed 's/^/  /'
    if ask_yn "setup: comment those lines out with the prefix '$SUPERSEDED_PREFIX'? [y/N]" n; then
      bashrc_comment_old "$stage"
    fi
  fi
  staged_write "$rc" "$stage" 0644 || true
  rm -f "$stage"
}
...
step_bashrc() {
  ...
  if [ "$SEC_AUTO" = 1 ]; then
    ...
    ask_choice "setup: install shell functions (~/.bashrc-cbox + ~/.zshrc-cbox and marker blocks in ~/.bashrc, ~/.zshrc)" "$CBOX_BASHRC" 0 1
  ...
  if [ "$SEC_AUTO" = 1 ]; then
    note "auto: keeping CBOX_BASHRC_COMMANDS=${CBOX_BASHRC_COMMANDS:-all} (cbox always installed)"
  else
    local all=(claude codex hermes) name i out=() expanded
    expanded="$(canonical_expand "${CBOX_BASHRC_COMMANDS:-all}" "${all[*]}")"
    CB_ITEMS=("${all[@]}")
    CB_STATE=()
    CB_DISABLED=()
    CB_REASON=()
    for name in "${all[@]}"; do
      if [[ " $expanded " == *" $name "* ]]; then CB_STATE+=(y); else CB_STATE+=(n); fi
      if [ "$name" = hermes ] && [ "${CBOX_HERMES:-off}" != on ]; then
        CB_DISABLED+=(1); CB_REASON+=("enable the hermes engine first")
      else
        CB_DISABLED+=(0); CB_REASON+=("")
      fi
    done
    note "select which engine wrappers to install (cbox is always installed)"
    checkbox_select
    for i in "${!CB_ITEMS[@]}"; do
      if [ "${CB_STATE[i]}" = y ] && [ "${CB_DISABLED[i]}" != 1 ]; then out+=("${CB_ITEMS[i]}"); fi
    done
    if [ "${#out[@]}" -eq 0 ]; then
      CBOX_BASHRC_COMMANDS="none"
    else
      CBOX_BASHRC_COMMANDS="$(canonical_store "${out[*]-}" "${all[*]}")"
    fi
  fi
  local body
  body="$(mktemp)"
  gen_bashrc > "$body"
  _install_rc_block "$HOME/.bashrc" "$HOME/.bashrc-cbox" '$HOME/.bashrc-cbox' "$body"
  _install_rc_block "$HOME/.zshrc" "$HOME/.zshrc-cbox" '$HOME/.zshrc-cbox' "$body"
  rm -f "$body"
  note "warning: the claude/codex functions shadow host binaries in new shells"
  note "the shell functions route through '$INSTALL_DIR/cbox run' for lifecycle management; reload your shell after this run (source ~/.bashrc or ~/.zshrc)"
}
...
autoresume_ensure_hooks() {
  [ "$CBOX_LIMIT_AUTORESUME" = on ] || [ "$CBOX_SAFEGUARD_AUTOCONFIRM" = on ] || return 0
  [ "$CBOX_CLAUDE_MODE" = mount ] || return 0
  [ -d "$ETC_DIR/hooks" ] || return 0
  container_target_ok || return 0
  gen_hooks_dir
...
step_autoresume() {
  ...
  local prev_safeguard="$CBOX_SAFEGUARD_AUTOCONFIRM"
  ask_choice "setup: auto-confirm the model-safeguard switch dialog (tmux-wrapped claude sessions, watchdog answers it so work does not stall)" "$CBOX_SAFEGUARD_AUTOCONFIRM" off on
  CBOX_SAFEGUARD_AUTOCONFIRM="$ASK_VALUE"
  if [ "$CBOX_SAFEGUARD_AUTOCONFIRM" = on ] && [ "$CBOX_CLAUDE_MODE" != mount ]; then
    warn "safeguard auto-confirm needs claude mount mode (watchdog lives in ~/.claude/hooks); keeping off"
    CBOX_SAFEGUARD_AUTOCONFIRM=off
  fi
  if [ "$CBOX_SAFEGUARD_AUTOCONFIRM" = on ] && [ "${CBOX_SESSION_SCOPE:-}" = global ]; then
    warn "safeguard auto-confirm needs an isolated session scope (the watchdog env is injected only for isolated projects); it will be inert under a global scope"
  fi
  ...
  if [ "$CBOX_LIMIT_AUTORESUME" = "$prev" ] && [ "$CBOX_SAFEGUARD_AUTOCONFIRM" = "$prev_safeguard" ]; then
    return 0
  fi
  autoresume_ensure_hooks
...
codex_mcp_apply() {
  ...
  if [ -f /.dockerenv ]; then
    ...
    note "container not running; codex volume config not touched - start it and re-run cbox setup update codex-mcp"
  fi
  rm -rf "$stage"
}
...
user_dir_precreate_host() {
  local user_dir="${CBOX_USER_DIR-$HOME/.config/cbox/user}"
  [ -n "$user_dir" ] || return 0
  mkdir -p "$user_dir/mcp" "$user_dir/policies"
}
...
claude_md_user_policies_block_file() {
  local out="$1" user_dir="${2-}" f b
  : > "$out"
  [ -n "$user_dir" ] || return 1
  local -a files=()
  while IFS= read -r f; do
    [ -n "$f" ] || continue
    files+=("$f")
  done < <(_cbox_user_policies_files "$user_dir/policies")
  [ "${#files[@]}" -gt 0 ] || return 1
  {
    printf '%s\n' "$CLAUDE_MD_USERPOL_MARK_START"
    for f in "${files[@]}"; do
      b="$(basename "$f")"
      printf '@~/.claude/policies/user/%s\n' "$b"
    done
...
claude_md_merge_user_policies_block() {
  local file="$1" user_dir="${2-}" block tmp has_content=1
  block="$(mktemp)"
  if ! claude_md_user_policies_block_file "$block" "$user_dir"; then
    has_content=0
  fi
  tmp="$(mktemp)"
  local has_span=0
  if grep -qF "$CLAUDE_MD_USERPOL_MARK_START" "$file" && grep -qF "$CLAUDE_MD_USERPOL_MARK_END" "$file"; then
    has_span=1
  fi
  if [ "$has_content" = 1 ]; then
    if [ "$has_span" = 1 ]; then
      awk -v s="$CLAUDE_MD_USERPOL_MARK_START" -v e="$CLAUDE_MD_USERPOL_MARK_END" -v bf="$block" '
        index($0, s) { skip = 1; while ((getline line < bf) > 0) print line; close(bf); next }
        index($0, e) { skip = 0; next }
        skip { next }
        { print }
      ' "$file" > "$tmp"
      if grep -qF "$CLAUDE_MD_KERNEL_MARK_START" "$tmp"; then
        local userpol_line kernel_line
        userpol_line="$(grep -nF "$CLAUDE_MD_USERPOL_MARK_START" "$tmp" | head -n1 | cut -d: -f1)"
        kernel_line="$(grep -nF "$CLAUDE_MD_KERNEL_MARK_START" "$tmp" | head -n1 | cut -d: -f1)"
        if [ -n "$userpol_line" ] && [ -n "$kernel_line" ] && [ "$userpol_line" -gt "$kernel_line" ]; then
          local reordered stripped
          reordered="$(mktemp)"
          stripped="$(mktemp)"
          awk -v s="$CLAUDE_MD_USERPOL_MARK_START" -v e="$CLAUDE_MD_USERPOL_MARK_END" '
            index($0, s) { skip = 1; next }
            index($0, e) { skip = 0; next }
            skip { next }
            { print }
          ' "$tmp" > "$stripped"
          awk -v s="$CLAUDE_MD_KERNEL_MARK_START" -v bf="$block" '
            index($0, s) && !done { while ((getline line < bf) > 0) print line; close(bf); done = 1 }
            { print }
          ' "$stripped" > "$reordered"
          mv "$reordered" "$tmp"
          rm -f "$stripped"
    ...
    elif grep -qF "$CLAUDE_MD_KERNEL_MARK_START" "$file"; then
      awk -v s="$CLAUDE_MD_KERNEL_MARK_START" -v bf="$block" '
        index($0, s) && !done { while ((getline line < bf) > 0) print line; close(bf); done = 1 }
        { print }
      ' "$file" > "$tmp"
    ...
    if [ "$has_span" = 1 ]; then
      awk -v s="$CLAUDE_MD_USERPOL_MARK_START" -v e="$CLAUDE_MD_USERPOL_MARK_END" '
        function isblank(l) { return l ~ /^[[:space:]]*$/ }
        index($0, s) {
          skip = 1
          after_span = 1
          next
        ...
        after_span {
          after_span = 0
          if (isblank($0) && pending_blank) { pending_blank = 0; next }
        }
        isblank($0) {
        ...
        END {
          if (pending_blank && !after_span) print prev_blank
        }
      ' "$file" > "$tmp"
    else
      cp "$file" "$tmp"
...
claude_user_policies_symlink() {
  ...
  mkdir -p "$CBOX_CLAUDE_PATH/policies"
  local link="$CBOX_CLAUDE_PATH/policies/user"
  if [ -L "$link" ]; then
    ln -sfn "$user_dir/policies" "$link"
  elif [ ! -e "$link" ]; then
    ln -s "$user_dir/policies" "$link"
  else
    echo "cbox: $link exists and is not a symlink - user policy imports skipped; move it aside" >&2
    return 1
  fi
}
...
step_claude_md() {
  ...
  if [ "$CBOX_CLAUDE_MODE" = mount ]; then
    ...
    local userpol_dir="${CBOX_USER_DIR-$HOME/.config/cbox/user}"
    if claude_user_policies_symlink && [ -n "$userpol_dir" ]; then
      claude_md_merge_user_policies_block "$work" "$userpol_dir"
    else
      claude_md_merge_user_policies_block "$work" ""
    fi
    ...
    claude_md_merge_user_policies_block "$work" "${CBOX_USER_DIR-$HOME/.config/cbox/user}"
    claude_md_merge_kernel_block "$work"
...
step_continuity() {
  ...
  if [ "$CBOX_HISTORY" = 0 ]; then
    ...
    if [ "$SEC_AUTO" != 1 ]; then
      note "switches saved; run cbox setup update claude-md and cbox setup update agents to deploy the change"
    fi
    return 0
  fi
  if [ "$SEC_AUTO" != 1 ]; then
...
ssh_mixed_sync() {
  ...
  if container_running; then
    ...
    note "container not running; ssh config for mixed mode not synced (re-run: cbox setup update ssh)"
  fi
}
...
run_wizard() {
  ...
  while [ "$i" -lt "${#WIZ_SECTIONS[@]}" ]; do
    ...
    case "$NAV" in
      ...
      quit)
        conf_save
        note "saved to $CONF_FILE; run cbox setup again to continue"
        return 0
        ;;
      *)
        if [ "$((NAV-1))" -ne "$i" ]; then
...
run_rebless() {
  SETUP_MODE=update
  [ -f "$CONF_FILE" ] || die "no $CONF_FILE; run cbox setup first"
  conf_load
  load_generators
  user_dir_precreate_host
  regen_all
...
run_update() {
  ...
  if [ "$section" = workspaces ] && [ "$CBOX_CLAUDE_MODE" = mount ]; then
    staged_install_files "$GEN_DIR/hooks" "$CBOX_CLAUDE_PATH/hooks" 0644 codex_scope.container.json \
      || note "codex guard scope not synced; run cbox setup update hooks (or ./cbox install-hooks) before relying on the new workspace list"
  fi
...
run_config() {
  ...
  if [ "$CBOX_CLAUDE_MODE" = mount ]; then
    note "host-write sections are skipped in --config mode (bashrc mcp-servers agents claude-md settings hooks); apply them later with cbox setup update <section>"
  else
  ...
  if [ "$CBOX_CODEX_MCP" = 1 ] && [ "$CBOX_CLAUDE_MODE" = mount ]; then
    note "codex-mcp is enabled but config writes (including the hooks dependency) are skipped in --config mode; apply with cbox setup update codex-mcp"
  fi
  if [ "$CBOX_HISTORY" = 1 ] && [ "$CBOX_CLAUDE_MODE" = mount ]; then
    note "continuity hooks staging is skipped in --config mode; apply with cbox setup update continuity"
  fi
  if [ "$CBOX_CODEX_MODE" = mount ]; then
    note "codex managed profile host files (~/.codex/cbox-container.config.toml, AGENTS.override.md, cbox-host.config.toml) are skipped in --config mode; apply with cbox setup update hooks"
  fi
  user_dir_precreate_host
  conf_save
  regen_all
...
_uninstall_rc_block() {
  local rc="$1" rc_cbox="$2" stage
  [ -f "$rc" ] || return 0
  grep -qF "$MARK_START" "$rc" || return 0
  stage="$(mktemp -d)"
  awk -v s="$MARK_START" -v e="$MARK_END" '
    ...
    { print }
  ' "$rc" > "$stage/rc"
  staged_write "$rc" "$stage/rc" 0644 || true
  rm -rf "$stage"
  note "kept $rc_cbox; delete it manually if you no longer want the shell functions"
}

uninstall_bashrc() {
  _uninstall_rc_block "$HOME/.bashrc" "$HOME/.bashrc-cbox"
  _uninstall_rc_block "$HOME/.zshrc" "$HOME/.zshrc-cbox"
}
...
_cbox_setup_main() {
  case "${1:-}" in
    ...
    list-steps)
      ...
      ;;
    --config)
      [ -n "${2:-}" ] || die "usage: cbox setup --config <file>"
      run_config "$2"
      ;;
    --local)
